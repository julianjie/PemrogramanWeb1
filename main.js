document.getElementById("simpan").addEventListener(
    ...
    function () {
      ...
      for (const [idx, dt] of listmhs.entries()) {
        data +=
          ...
          dt.nama +
          "</td>" +
          "<td><button class = 'btn btn-danger' onClick='deleteData(" + idx + ")'>Delete</button></td>"+
          "</tr>";
      }
  ...
  function deleteData(index){
    let list = JSON.parse(localStorage.getItem("mhs"));
    list.splice(index,1);
    localStorage.setItem("mhs", JSON.stringify(list));
    renderData();
  }
